...
class Readability {
    ...
    // 需要删除的标签
    private $junkTags = Array("style", "form", "iframe", "script", "button", "input", "textarea", "li", "script");
    ...
    private function getTopBox() {
        ...
        while($paragraph = $allParagraphs->item($i++)) {
            ...
            if (preg_match("/(comment|meta|footer|footnote|secondary_nav|logo|logo_links|top_info|pane_most_read_today|popular_today)/i", $className)) {
                $contentScore -= 100;
            } else if(preg_match(
            ...
            if (preg_match("/(comment|meta|footer|footnote|popular_today|right_panel|footer_2)/i", $id)) {
                $contentScore -= 100;
            } else if (preg_match(
