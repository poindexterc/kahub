...
*/
    ...
    class YouTube{
        ...
        function generateEmbedded( $id, $width = 610, $height = 343 ){
            $randID= rand(0,700);
            ...
            return  '<div class="videoInline" id="vidInline-'.sha1(md5($id).$randID).'"><div class="placeholderImg" onClick="jQuery(this).hide(); jQuery(\'#vidEmbed-'.sha1(md5($id).$randID).'\').show(); jQuery(\'#vidInline-'.sha1(md5($id).$randID).'\').height(343);"><p class="playVid">Play&rang;</p><img src="http://img.youtube.com/vi/'.$id.'/0.jpg"></div><div id="vidEmbed-'.sha1(md5($id).$randID).'" class="vidEmbed">'.$embed.'</div></div>';
    ...
    class Vimeo{
        ...
        function generateEmbed($video_url){
            ...
            $xml_url = $oembed_endpoint . '.xml?url=' . rawurlencode($video_url) . '&width=610&autoplay=true&iframe=false';
            $oembed = simplexml_load_string(Vimeo::curl_get($xml_url));
            $randID= rand(0,700);
            $thumb = html_entity_decode($oembed->thumbnail_url);
            $embed = html_entity_decode($oembed->html);
            $id = html_entity_decode($oembed->video_id);
            return '<div class="videoInline" id="vidInline-'.sha1(md5($id).$randID).'"><div class="placeholderImg" onClick="jQuery(this).hide(); jQuery(\'#vidEmbed-'.sha1(md5($id).$randID).'\').show(); jQuery(\'#vidInline-'.sha1(md5($id).$randID).'\').height(343);"><p class="playVid">Play&rang;</p><img src="'.$thumb.'"></div><div id="vidEmbed-'.sha1(md5($id).$randID).'" class="vidEmbed">'.$embed.'</div></div>';
        }
    }
