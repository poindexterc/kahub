...
class flexibleAccess{
	...
	function flexibleAccess($dbConn = '', $settings = '')
	{
		...
		$this->dbName = Settings::GetDatabaseName();		
		if($this->dbConn == '')
		{
			$this->dbConn = Settings::ConnectDB();			
		}
		$db_selected = mysql_select_db($this->dbName, $this->dbConn) or die(mysql_error());
		if($db_selected)
		{
	...
	*/
	function login($uname, $password, $remember = false, $loadUser = true)
	{
		...
		if ( $loadUser )
		{
			...
			/*$result = mysql_query("SELECT roles.RoleName 
								FROM userroles 
										INNER JOIN member ON userroles.MemberID = member.ID INNER JOIN 
										roles ON userroles.RoleID = roles.RoleID 
								WHERE  (member.ID = " . $this->userID . ")");
			$row = mysql_fetch_array($result);
			while($row != false)
			{
				$this->userRoles[] = $row['RoleName'];
				$row = mysql_fetch_array($result);
			}*/
	...
	/**
	 * User Is In Role Function to find if current user is in role specified
	...
	*/
	...
	{
		$result = false;
		$userID = $_SESSION[$this->sessionVariable];
		$res = $this->query("SELECT roles.RoleName 
							 FROM userroles 
									INNER JOIN roles ON userroles.RoleID = roles.RoleID 
							 WHERE  (userroles.MemberID = " . $this->userID . ") AND (roles.RoleName = '" . $roleName . "')",__LINE__);
