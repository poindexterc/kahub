...
class thumbnail_image {
	...
	function GenerateThumbFile($from_name, $to_name) 
	{
		...
		if (substr($from_name,0,7) == 'http://') 
		{
			...
			if ($temp) 
			{
				//fwrite($temp, file_get_contents($from_name)) or die("Cannot download image"); // original line
				//fclose($temp); // original line
				//$from_name = $tmpfname; // original line
				
				// create curl resource 
				$ch = curl_init(); 
				
				// set url 
				curl_setopt($ch, CURLOPT_URL, $from_name); 
				// set user agent
				...
				fwrite($temp, $output) or die("Cannot download image");
				fclose($temp);
				$from_name = $tmpfname;
			}
			else 
