...
{	
	...
	function LatestStoriesHubArray($userID){
		...
		$Query = "SELECT DISTINCT s.StoryID, s.Story_Title, s.Story_URL, s.views, s.DateTime, s.ImageID, c.MemberID, c.Comment_Text, c.CommentID, c.Date_Time, c.PostType
					FROM tbl_comments c
					INNER JOIN tbl_story s ON c.StoryID = s.StoryID
					WHERE (c.MemberID = '$userID') AND (c.Comment_Text!='Promote') AND (c.PostType!=1)
					ORDER BY s.DateTime DESC";
		$QueryResult =  mysql_query($Query)or die(mysql_error());
		$row = mysql_fetch_array($QueryResult);
		$MemberID = $GLOBALS['user']->userID;
