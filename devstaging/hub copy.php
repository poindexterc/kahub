...
$user = new flexibleAccess();
	...
	if ($isHub!=1){
		header('Location:http://www.connichiwah.com/member/profile.php?user='.$userNum);	
	} 
	...
	$stories = HelpingDBMethods::GetLatestStoriesPerson(0, 50, $userNum);
	$badges = HelpingDBMethods::GetBadgesProfile($userNum, "profile", 14, 0);
	...
	echo $fname.' Hub';
	if ($friend!=true){
					echo '<span class="addsource"><input type="button" value="    Follow" onclick="Followhub('.$userNum.');" class="btn-follow"></span>';
	}
	echo '</div>';
if ($friend!=true){
	echo '<div class="nothing"><span class="nothing-name">This is a hub!</span>  Hubs keep you updated with the latest, gretest, breaking stories about a topic. With hubs, you can discover new and cool stuff, as curated by the connichiwah team. Once you follow a hub, stories published by the hub will start appearing in your Latest News feed.</div>';
	echo '<div class="profile-latest">Latest stories about '.$first.'</div>';
	echo $stories;
} else if ($friend=true)  {
		echo '<div class="profile-latest">Latest stories about '.$first.'</div>';
		echo $stories;
