...
var readability = {
	 ...
	 **/
	regexps: {
		unlikelyCandidatesRe:   /combx|comment|disqus|foot|header|menu|meta|nav|rss|shoutbox|sidebar|sponsor|head/i,
	...
	init: function(preserveUnlikelyCandidates) {
		...
		var articleContent1 = readability.grabArticle(preserveUnlikelyCandidates);
		var articleContent = articleContent1;
	...
	prepDocument: function () {
		...
		for (var k=0;k < document.styleSheets.length; k++) {
			if (document.styleSheets[k].href != null && document.styleSheets[k].href.lastIndexOf("readability") &&  document.styleSheets[k].href.lastIndexOf("connichi_styles-frame") == -1) {
	...
	prepArticle: function (articleContent) {
		...
		readability.clean(articleContent, "head");
		/**
	...
	cleanHeaders: function (e) {
		for (var headerIndex = 1; headerIndex < 7; headerIndex++) {
			...
			for (var i=headers.length-1; i >=0; i--) {
				if (readability.getClassWeight(headers[i]) < 0 || readability.getLinkDensity(headers[i]) > 0.43) {
