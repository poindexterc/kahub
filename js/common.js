...
$myjquery(document).ready(function(){
    ...
    $myjquery(".more").click(function(){
        ...
        if(!isRequesting)
        {
			...
			var view = parseInt($myjquery("#viewtype").val());
            ...
            if(offset < totlalResults)
            {
                $myjquery.ajax({
                    ...
                    data: "limit=" + limit + "&offset=" + offset + "&viewtype" + view,
					alert(view);
         ...
         if(view==1){
				$myjquery(".more").hide();
                $myjquery(".more-message").html("Older Stories Temporarily Unavailable");
                $myjquery(".more-message").show();
			}
        }
    });
...
//                            $myjquery("#message-div").html(data);
                            AttachMessageDivEvents(data, 'Source Add')
		            	}
                    }
                });
    }
    ...
    $myjquery.ajax(
            {
                ...
                {
//                    $myjquery("#message-div").html(data);                    
                    AttachMessageDivEvents(data, 'Sources Added');
...
{
	document.getElementById("story-"+storyid).innerHTML="<span class='Interesting' id='done'>Added to Read Later!</span>";
	$myjquery("#story-"+storyid).html('<span class="Interesting" id="done">Added to Read Later!</span>');
	console.log("liked");
...
function GetStoryData(storyid)
{
	...
	$myjquery(".dark-gray-img").html("<img src='http://www.connichiwah.com/member/placeholderImage.png'>");
    ...
    $myjquery.ajax(
        {            
            ...
            success: function(data)
            {
                $myjquery("#dark-gray-heading").html("<a href = 'http://www.connichiwah.com/anyShare/index.php?ref=hero_headline&rand=0&rURL=" + $myjquery(data).find('url').text() + "' target='_blank'>" + $myjquery(data).find('stitle').text() + "</a>");
                ...
                $myjquery(".box-member-comment-text").html($myjquery(data).find('comment').text() + "<a class = 'box-member-comment-read-more' href = 'http://www.connichiwah.com/anyShare/index.php?ref=hero_read_more&rand=0&rURL=" + $myjquery(data).find('url').text() + "'  target='_blank'>read more...</a>");
                if(tags.length >0) 
                {
    ...
    $myjquery(data.photos.photo).each(function(index){
        ...
        else
        {
            var src = "http://farm" + this.farm +
                      ...
                      "_" + this.secret +
                      "_s.jpg";
            $myjquery("#flicker-images").append('<img src = "' +  src + '" alt = "" onerror="this.src=\'http://www.connichiwah.com/member/placeholderImageSmall.png\'"/>');
        }
