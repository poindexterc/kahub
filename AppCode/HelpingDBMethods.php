...
include '../anyShare/youtube.php';
include '../anyShare/youtube2.php';
...
{	
	...
	function spamCheck($data){
		...
		$spamDomains = explode(",", "ly,gl,com,tk,hk,www,http,cc,ro,cm,su,ru,sg,ch,cn,info,net,biz,ph");
		...
		foreach($dataArray as $item){
			...
			$spamDomainArray = explode(".", $item);
			foreach($spamDomainArray as $part){
			    if(in_array($part, $spamDomains)){
			        $akismet = "true";
			    }
			}
		}
	...
	{
		$Query = "INSERT INTO tbl_story (Story_Title, Story_URL) VALUES  ('" . mysql_real_escape_string($title) . "', '" . $url . "')";
		$QueryResult =  mysql_query($Query)or die(mysql_error());
	}
	...
	function PostComments($StoryID, $text_selected, $Comments, $MemberID, $ReplyTo)
	{
		...
		$data = array('blog' => 'http://www.kahub.com',
		              ...
		              'comment_content' => urldecode($Comments));
		...
		if($spam=="true"){
			...
			if($spamScore>7){
			    HelpingDBMethods::banUser($MemberID, 2);
			}
		} else {
			$Query = "INSERT INTO tbl_comments (StoryID, Comment_Associated_Text, Comment_Text,  MemberID, Reply_To) 
					VALUES (" . $StoryID . ", '" . $text_selected . "', '" . mysql_real_escape_string(urldecode($Comments)) . "', " . $MemberID . ", " . $ReplyTo . ")";
	...
	function PostCommentsTopic($StoryID, $text_selected, $Comments, $MemberID, $ReplyTo, $topic)
	{
		$name = MemberDBMethods::GetUserName($MemberID);
		$email = MemberDBMethods::GetEmail($MemberID);
		$storyData = HelpingDBMethods::GetStoryData($StoryID);
		...
		if($spam=="true"){
			$Query = "INSERT INTO tbl_comments (StoryID, Comment_Associated_Text, Comment_Text,  MemberID, Reply_To, PostTag) 
					VALUES (" . $StoryID . ", '" . $text_selected . "', '" . mysql_real_escape_string(urldecode($Comments)) . "', " . $MemberID . ", " . $ReplyTo . ", '".$topic."')";
			$QueryResult =  mysql_query($Query)or die(mysql_error());
			$commentID = mysql_insert_id();
			HelpingDBMethods::submitSpam($StoryID, $MemberID, $commentID, $Comments);
			$spamScore = HelpingDBMethods::spamScore($MemberID);
	...
	function PostVideo($StoryID, $text_selected, $Comments, $MemberID, $ReplyTo)
	{
		$name = MemberDBMethods::GetUserName($MemberID);
		$email = MemberDBMethods::GetEmail($MemberID);
		$storyData = HelpingDBMethods::GetStoryData($StoryID);
		...
		if($spam=="true"){
			$Query = "INSERT INTO tbl_comments (StoryID, Comment_Associated_Text, Comment_Text,  MemberID, Reply_To, PostType) 
					VALUES (" . $StoryID . ", '" . $text_selected . "', '" . mysql_real_escape_string(urldecode($Comments)) . "', " . $MemberID . ", " . $ReplyTo . ", 2)";
			$QueryResult =  mysql_query($Query)or die(mysql_error());
			$commentID = mysql_insert_id();
			HelpingDBMethods::submitSpam($StoryID, $MemberID, $commentID, $Comments);
			$spamScore = HelpingDBMethods::spamScore($MemberID);
	...
	function PostPhoto($StoryID, $text_selected, $Comments, $MemberID, $photo)
	{
		$name = MemberDBMethods::GetUserName($MemberID);
		$email = MemberDBMethods::GetEmail($MemberID);
		$storyData = HelpingDBMethods::GetStoryData($StoryID);
		...
		$spam=HelpingDBMethods::spamCheck($data);
		$DateTime = gmdate("Y-m-d H-i-s");//date('Y-m-d H-i-s')
		list($width, $height, $type, $attr)=getimagesize('http://www.kahub.com/l/'.$photo); 
        $ht=$height; 
        $wd=$width;
        if($wd<$ht){
            $type = 2;
		...
		if($spam=="true"){
			$Query = "INSERT INTO tbl_comments (StoryID, Comment_Associated_Text, Comment_Text,  MemberID, Reply_To, PostType) 
					VALUES (" . $StoryID . ", '" . $text_selected . "', '" . mysql_real_escape_string(urldecode($Comments)) . "', " . $MemberID . ",0, 3)";
			$QueryResult =  mysql_query($Query)or die(mysql_error());
			$commentID = mysql_insert_id();
			$photoQuery = "INSERT INTO tbl_buckets (ImageSrc, commentID, ImageType,  MemberID) 
					VALUES (" . mysql_real_escape_string(urldecode($photo)) . ", " . $commentID . ", " . $type . ", " . $MemberID . ")";
			$photoQueryResult =  mysql_query($photoQuery)or die(mysql_error());
			$photocommentID = mysql_insert_id();
			HelpingDBMethods::submitSpam($StoryID, $MemberID, $commentID, $Comments);
			$spamScore = HelpingDBMethods::spamScore($MemberID);
			if($spamScore>7){
			    HelpingDBMethods::banUser($MemberID, 2);
	...
	function banUser($MemberID, $banType)
	{
		$ip = $_SERVER['REMOTE_ADDR'];
		$Query = "INSERT INTO tbl_ban (MemberID, IP, BanType) 
				VALUES (" . $MemberID . ", '" . $ip . "', " . $banType . ")";
		$QueryResult =  mysql_query($Query)or die(mysql_error());
	}
	...
	function insertStoryTextExtension($storyBody, $url, $title)
	{
		...
		if($storyID==0){
			...
			$storyBodyesc = mysql_real_escape_string($storyBody);
			$Query = "INSERT INTO tbl_story_text (StoryID, Story_URL, Story_Body) 
					VALUES ('" . $storyID . "', '" . mysql_real_escape_string(urldecode($url)) . "', '$storyBodyesc')";
			$QueryResult =  mysql_query($Query)or die(mysql_error());
	}
	...
	function getOfficialSong($song)
	{
		$result = 0; 
		$Query = "SELECT storyID FROM tbl_musicVideo WHERE (songName  = '$song')";	
		$QueryResult =  mysql_query($Query)or die(mysql_error());
		$row = mysql_fetch_array($QueryResult);
		if($row!=false)
		{
	...
	function GetFollowersNoNameAll($MemberID)
	{
		$result = ''; 
		$rootURL = Settings::GetRootURL();
		$Query = "SELECT m.* 
		...
		while($row!=false)
		{
			...
			$pal = HelpingDBMethods::isMyFriend($MemberID,$hubData['h-memberID']);
			if(!$following&&!$pal){
				$follow = '<a class="friendsPage btn-add-prof" onclick="Followhub('.$hubData['h-hubID'].');">Follow</a>';
			} else {
				$follow = '';
			}
	...
	function GetLatestStories($offset, $limit, $userID, $getTotlaResults = false, &$totlalResults = null)
	{
		...
		$result = "";
		$Query = "SELECT DISTINCT s.StoryID, s.Story_Title, s.Story_URL, s.views, s.DateTime, s.ImageID, c.PostType, c.MemberID, c.Comment_Text, c.CommentID, c.Reply_To, c.Date_Time, c.PostTag
					FROM tbl_friends a
					INNER JOIN tbl_friends b ON a.MemberID_Active = b.MemberID_Passive AND a.MemberID_Passive = b.MemberID_Active
					INNER JOIN tbl_comments c ON a.MemberID_Active = c.MemberID OR a.MemberID_Passive = c.MemberID
					INNER JOIN tbl_story s ON c.StoryID = s.StoryID
		...
		while($row != false)
		{
			...
			$replyCount = HelpingDBMethods::GetReplyCount($row['CommentID']);
			if($row['PostTag']!=""){
			    $tid = HelpingDBMethods::getTopicHubID($row['PostTag']);
			    $tag = "<span class='tag'> &mdash; about <a href='http://www.kahub.com/l/hub.php?user=".$tid."'>".$row['PostTag']."</a>.</span>";
			} else {
			    $tag = "";
			}
			if(!$isLiked)
			{
				...
				$interesting = '<span class="Interesting" onclick="LikeStory('.$storyID.',\''.$urlhex.'\')" id="story-'.$storyID.'">Read Later</span>';
				$later="noLater";
			...
			if(!$isPromoted){
				$promote = "<span class='promoteLoad' onClick='promoteShow(".$storyID.")'><a onClick=\"PromoteStory(".$storyID.", '".$urlhex."'); jQuery('#comments-box-".$row['CommentID']."').show();\" class='promolink' id='promote-story-link-".$storyID."'>Promote</a></span> (<span class='promo-num' id='promo-num-".$storyID."'>".$promoCount."</span>)";
			} else {
            ...
            if($row['PostType']==2&&$isBanned!="shadow"&&$comment!=""){
                $raw_url = parse_url($row['Story_URL']); 
            	$domain_only =str_replace ('www.','', $raw_url); 
            	$domain =  $domain_only['host'];
            	if($domain=="youtube.com"){
            		$yt  = new YouTube();
            		$vidid = $yt->getId($row['Story_URL']);
            		$storyText = $yt->generateEmbedded($vidid);
            	} else if($domain=="vimeo.com") {
            	    $vimeo = new Vimeo();
            	    $storyText = $vimeo->generateEmbed($row['Story_URL']);
            	}
            		$result .= '<li class = "vidPost '.$later.'" onclick=getStoryInfo('.$storyID.')>'.$storyText.''.$imgString . '<a href = "' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><h2 style = "padding-top:8px;" class = "heavy titles">' . HelpingMethods::GetLimitedText($row['Story_Title'], 70) . '</h2></a><div class="attrib"><span class="uname" ><a href="http://www.kahub.com/l/profile.php?user='.$mid.'">'.$name.'</a></span><span class="commentstart"> said <span class="comment">'.HelpingMethods::GetLimitedText($comment, 250).' </span> in a video post.</span></div><div class="others"><span class="promote">'.$promote.'</span> &#183; <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$commentsBox.'</li><div class="clear"></div>
    			';
            }else if($row['PostType']==2&&$isBanned!="shadow"){
                $raw_url = parse_url($row['Story_URL']); 
            	$domain_only =str_replace ('www.','', $raw_url); 
            	$domain =  $domain_only['host'];
            	if($domain=="youtube.com"){
            		...
            		$result .= '<li class = "vidPost ' . $rankClass . ' '.$later.'" onclick=getStoryInfo('.$storyID.')>'.$storyText.''.$imgString . '<a href = "' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><h2 style = "padding-top:8px;" class = "heavy titles">' . HelpingMethods::GetLimitedText($row['Story_Title'], 70) . '</h2></a><div class="attrib"></div><div class="others"><span class="promote">'.$promote.'</span> &#183; <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$commentsBox.'</li><div class="clear"></div>
    			';
    		}else if($row['PostType']==3&&$isBanned!="shadow"&&$comment!=""){
                        $photoInfo = HelpingDBMethods::getPhotoInfo($row['CommentID']);
                        if($photoInfo['i-type']==1){
                        		$result .= '<li class = "photoPost '.$later.'" onclick=getStoryInfo('.$storyID.')><div class="photoType1Preview"><a href="http://www.kahub.com/l/'.$photoInfo['i-src'].'" rel="gallery" title="'.$row['Story_Title'].'"><img src="http://www.kahub.com/l/'.$photoInfo['i-src'].'"></a></div>'.$imgString . '<a href = "' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><h2 style = "padding-top:8px;" class = "heavy titles">' . HelpingMethods::GetLimitedText($row['Story_Title'], 70) . '</h2></a><div class="attrib"><span class="uname" ><a href="http://www.kahub.com/l/profile.php?user='.$mid.'">'.$name.'</a></span><span class="commentstart"> said <span class="comment">'.HelpingMethods::GetLimitedText($comment, 250).' </span> in a photo post.</span></div><div class="others"><span class="promote">'.$promote.'</span> &#183; <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$commentsBox.'</li><div class="clear"></div>';
                        } else {
                            $storyText = HelpingMethods::GetLimitedText(HelpingDBMethods::GetStoryText($storyID), 200);
                            $result .= '<li class = "photoPost '.$later.'" onclick=getStoryInfo('.$storyID.')><div class="photoType2Preview"><a href="http://www.kahub.com/l/'.$photoInfo['i-src'].'" rel="gallery" title="'.$row['Story_Title'].'"><img src="http://www.kahub.com/l/'.$photoInfo['i-src'].'"></a></div><div class="commentPhoto2"><div class="bigQuote">&ldquo;</div>'.HelpingMethods::GetLimitedText($comment, 250).' </div><div class="authorBigQuote">'.$imgString . '<div class="attrib bigquote"><span class="uname bigquote" ><a href="http://www.kahub.com/l/profile.php?user='.$mid.'">'.$name.'</a></span></div></div><br clear="all" /><a href = "' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><h2 style = "padding-top:8px;" class = "titles photo2">' . HelpingMethods::GetLimitedText($row['Story_Title'], 70) . '</h2></a><div class="storyExcerpt">'.$storyText.'</div><div class="others"><span class="promote">'.$promote.'</span> &#183; <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$commentsBox.'</li><div class="clear"></div>';
                        }
            }   else if($row['PostType']==3&&$isBanned!="shadow"){
                    $photoInfo = HelpingDBMethods::getPhotoInfo($row['CommentID']);
                    if($photoInfo['i-type']==1){
                        $result .= '<li class = "photoPost '.$later.'" onclick=getStoryInfo('.$storyID.')><div class="photoType1Preview"><a href="http://www.kahub.com/l/'.$photoInfo['i-src'].'" rel="gallery" title="'.$row['Story_Title'].'"><img src="http://www.kahub.com/l/'.$photoInfo['i-src'].'"></a></div>'.$imgString . '<a href = "' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><h2 style = "padding-top:8px;" class = "heavy titles">' . HelpingMethods::GetLimitedText($row['Story_Title'], 70) . '</h2></a><div class="attrib"></div><div class="others"><span class="promote">'.$promote.'</span> &#183; <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$commentsBox.'</li><div class="clear"></div>';
                    } else {
                        $storyText = HelpingMethods::GetLimitedText(HelpingDBMethods::GetStoryText($storyID), 700);
                        $result .= '<li class = "photoPost '.$later.'" onclick=getStoryInfo('.$storyID.')><div class="photoType2Preview"><a href="http://www.kahub.com/l/'.$photoInfo['i-src'].'" rel="gallery" title="'.$row['Story_Title'].'"><img src="http://www.kahub.com/l/'.$photoInfo['i-src'].'"></a></div><div class="authorBigQuote nocomment">'.$imgString . '<div class="attrib bigquote nocomment"><span class="uname bigquote nocomment" ><a href="http://www.kahub.com/l/profile.php?user='.$mid.'">'.$name.'</a></span></div></div><a href = "' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><h2 style = "padding-top:8px;" class = "titles photo2 nocomment">' . HelpingMethods::GetLimitedText($row['Story_Title'], 70) . '</h2></a><div class="storyExcerpt">'.$storyText.'</div><br clear="all" /><div class="others"><span class="promote">'.$promote.'</span> &#183; <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$commentsBox.'</li><div class="clear"></div>';
                    }
            }else if($replyID!=0 && $hub==0 && $comType==1 &&$isBanned!="shadow"){
			...
			';
				...
				';
			            ...
			            if($noComments<6){
			                ...
			                $result .= '<li class = "' . $rankClass . ' '.$later.'" onclick=getStoryInfo('.$storyID.')>'.$imgString . '<div class="storyText phub"><a href = "http://www.kahub.com/anyShare/xd_check.php?rURL=' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><div class="storyTextTitle">'.HelpingMethods::GetLimitedText($row['Story_Title'], 70) .'</div>'.$storyText.'</a></div></a><div class="attrib phub"><img src="http://www.kahub.com/images/promoteicn.gif" class="promoteImg"><a href="http://www.kahub.com/l/profile.php?user='.$mid.'">'.$name.'</a><span class="commentstart"><span class="comment"> promoted thier first stories!</span></span></div><div class="others"> <span class="promote">'.$promote.'</span> <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$commentsBox.'</li><div class="clear"></div>';
			            } else{
					...
					$result .= '<li class = "' . $rankClass . ' '.$later.'" onclick=getStoryInfo('.$storyID.')>'.$imgString . '<div class="storyText phub"><a href = "http://www.kahub.com/anyShare/xd_check.php?rURL=' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><div class="storyTextTitle">'.HelpingMethods::GetLimitedText($row['Story_Title'], 70) .'</div>'.$storyText.'</a></div></a><div class="attrib phub"><div class="uname phub" ><a href="http://www.kahub.com/'.$hubInfo['h-handle'].'">'.$hubName.'</a><span class="handle phub"> (/'.$hubInfo['h-handle'].') </span> <span class="headlinephub">'.$hubInfo['h-headline'].'</span> &#183; <span class="noFollowers phub">'.$noOfFriends.'</span></div><span class="commentstart"><span class="comment">'.HelpingMethods::GetLimitedText($comment, 250).'</span></span></div><div class="others"> <span class="promote">'.$promote.'</span> <span class="comment">'.$replyText.'</span> &#183; <span class="restate">'.$restate.'</span> &#183; <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$reply.$addComment.$hubComments.$commentsBox.'</li><div class="clear"></div>
							';
			}else if($mid!=$userID&&$isBanned!="shadow"){
				$storyText = HelpingMethods::GetLimitedText(HelpingDBMethods::GetStoryText($storyID), 200);
				if($storyText=="Looks like we couldn't find the content. :("){
					$result .= '<li class = "' . $rankClass . ' hubno '.$later.'" onclick=getStoryInfo('.$storyID.')><div class="noSummaryText"><a href="http://www.kahub.com/l/profile.php?user='.$mid.'">'.$name.'</a>  shared <a href = "http://www.kahub.com/anyShare/xd_check.php?rURL=' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')">' . HelpingMethods::GetLimitedText($row['Story_Title'], 30) . '</a></div><span class="others"><span class="promote">'.$promote.'</span> <span class="comment">'.$commentText.'</span> &#183; </span>'.$hubComments.'</li><div class="clear"></div></li>
					';
				} else {
								$result .= '<li class = "' . $rankClass . ' '.$later.'" onclick=getStoryInfo('.$storyID.')>'. $imgString . '<div class="storyText"><a href = "http://www.kahub.com/anyShare/xd_check.php?rURL=' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')"><div class="storyTextTitle">'.HelpingMethods::GetLimitedText($row['Story_Title'], 70) .'</div>'.$storyText.'</a></div><div class="others"><span class="promote">'.$promote.'</span> &#183; <span class="comment">'.$commentText.'</span> &#183;  <span class="read" onClick="readLater('.$storyID.')"> '.$interesting.'</span><span class="date"><a href="http://www.kahub.com/l/comments.php?c='.$row['CommentID'].'">'.HelpingDBMethods::time2str($date).'</a></span></div>'.$hubComments.'</li><div class="clear"></div>
					';
				}
	...
	function GetLatestStoriesInteresting($offset, $limit, $userID, $getTotlaResults = false, &$totlalResults = null)
	{
		...
		$result = "";
		$Query = "SELECT DISTINCT s.StoryID, s.Story_Title, s.Story_URL
					FROM tbl_friends a
					INNER JOIN tbl_friends b ON a.MemberID_Active = b.MemberID_Passive AND a.MemberID_Passive = b.MemberID_Active
					INNER JOIN tbl_comments c ON a.MemberID_Active = c.MemberID OR a.MemberID_Passive = c.MemberID
					INNER JOIN tbl_story s ON c.StoryID = s.StoryID
		...
		while( $row != false)
		{
			...
			if($isLiked)
			{
				...
				$result .= '<li class = "readLater" onclick=getStoryInfo('.$storyID.')><img src="http://www.google.com/s2/favicons?domain='.$domain.'" class="rlFavi"><a href = "http://www.kahub.com/anyShare/xd_check.php?ref=latest_list&rand=0&rURL=' . $row['Story_URL'] . '" target = "_blank" onclick = "StoryClick(' . $row['StoryID'] . ')" class="rlLink"><h2 style = "padding-top:8px;" class = "readLater">' . HelpingMethods::GetLimitedText($row['Story_Title'], 50) . '</h2></a></li>';
	...
	function LatestStoryTopic($userID){
		$Query = "SELECT StoryID FROM tbl_story WHERE (Story_Title LIKE '%$userID%')
	...
	function getMemberFromPhone($number){
		$Query = "SELECT MemberID FROM tbl_phones WHERE (phone = '$number');";
		$QueryResult =  mysql_query($Query)or die(mysql_error());
		$row = mysql_fetch_array($QueryResult);
		if($row != false)
		{
	...
	function addPhone($MemberID, $phone){
        $Query = "INSERT INTO tbl_phones (MemberID, phone) 
    			  VALUES (" . $MemberID . ", '" . mysql_real_escape_string($phone) . "')";		
    	$QueryResult =  mysql_query($Query)or die(mysql_error());
	}
	function getValidPassFromEmail($password){
		$Query = "SELECT * FROM tbl_member WHERE (mPassword = '$password');";
		$QueryResult =  mysql_query($Query)or die(mysql_error());
		$row = mysql_fetch_array($QueryResult);
		if($row != false)
		{
	...
	function LatestStoriesHubArray($userID){
		...
		$Query = "SELECT s.StoryID, s.Story_Title, s.Story_URL, s.views, s.DateTime, s.ImageID, c.MemberID, c.Comment_Text, c.CommentID, c.Date_Time, c.PostType
					FROM tbl_comments c
					INNER JOIN tbl_story s ON c.StoryID = s.StoryID
					WHERE (c.MemberID = '$userID') 
					ORDER BY s.DateTime DESC LIMIT 0,25";
		$QueryResult =  mysql_query($Query)or die(mysql_error());
		$row = mysql_fetch_array($QueryResult);
		$MemberID = $GLOBALS['user']->userID;
		while($row != false)
	...
	function LatestStoryPersonHub($userID, $handle){
		...
		if(!$isLiked)
		{
			$interesting = '<span class="Interesting" onclick="LikeStory('.$storyID.',\''.$urlhex.'\')" id="story-'.$storyID.'">Add to Read Later</span>';
		...
		if($row != false)
		{
			...
			<div class='lateststoryuname'><a href='http://www.kahub.com/".$hubData['h-handle']."'><span class='realName latest'>".$name."</span> <span class='handle latest'>(/".$handle.")</span></a></div>
	...
	function GetLatestStoriesPersonHub($array, $mid, $handle)
	{
		...
		foreach ($array as $storyID => $commentID)
		{
			...
			if($commentData['c-PostType']==2&&$commentData['c-text']!=""){
			    $raw_url = parse_url($storyData['s-url']); 
            	$domain_only =str_replace ('www.','', $raw_url); 
            	$domain =  $domain_only['host'];
            	if($domain=="youtube.com"){
            		$yt  = new YouTube2();
            		$vidid = $yt->getId($storyData['s-url']);
            		$video = $yt->generateEmbedded($vidid);
            	} else if($domain=="vimeo.com") {
            	    $vimeo = new Vimeo2();
                	$video = $vimeo->generateEmbed($storyData['s-url']);
                }
			    $result .= '<li onMouseOver="$myjquery(\'#pin-'.$commentID.'\').show()" onMouseOut="$myjquery(\'#pin-'.$commentID.'\').hide()" class="profile-stories phub2"><div class="showcaseImg">'.$video.'</div><br clear="all" /><a href = "http://www.kahub.com/anyShare/xd_check?rURL=' . $storyData['s-url'] . '" target = "_blank" onclick = "StoryClick(' . $storyID . ')"><p style = "padding-top:8px;" class = "heavy phub">' . HelpingMethods::GetLimitedText($storyData['s-title'], 70) . '</a><div class="storyText phubpage"></div><div class="name"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$imgString.'</a><span class="realName"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$name.'</a></span> <span class="handle">(/'.$handle.')</span></div><div class="comment phub2">'.$commentData['c-text'].' '.$pin.'</div><div class="else hub pleft"><span class="comment">'.$replyText.'</span> &#183; '.$restate.' <span class="promote">'.$promote.'</span></div>'.$reply.'<div class="repliesinlinephub">'.$commentsBox.'</li>';
			} else if($commentData['c-PostType']==2){
    			$raw_url = parse_url($storyData['s-url']); 
                $domain_only =str_replace ('www.','', $raw_url); 
                $domain =  $domain_only['host'];
                if($domain=="youtube.com"){
    			...
    			$result .= '<li onMouseOver="$myjquery(\'#pin-'.$commentID.'\').show()" onMouseOut="$myjquery(\'#pin-'.$commentID.'\').hide()" class="profile-stories phub2"><div class="showcaseImg">'.$video.'</div><br clear="all" /><a href = "http://www.kahub.com/anyShare/xd_check?rURL=' . $storyData['s-url'] . '" target = "_blank" onclick = "StoryClick(' . $storyID . ')"><p style = "padding-top:8px;" class = "heavy phub">' . HelpingMethods::GetLimitedText($storyData['s-title'], 70) . '</a><div class="storyText phubpage"></div><div class="name"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$imgString.'</a><span class="realName"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$name.'</a></span> <span class="handle">(/'.$handle.')</span></div><div class="else hub pleft"><span class="comment">'.$replyText.'</span> &#183; '.$restate.' <span class="promote">'.$promote.'</span></div>'.$reply.'<div class="repliesinlinephub">'.$commentsBox.'</li>';
    		} else if($commentData['c-PostType']==3&&$commentData['c-text']!=""){
    		     $photoInfo = HelpingDBMethods::getPhotoInfo($commentID);
 			    $result .= '<li onMouseOver="$myjquery(\'#pin-'.$commentID.'\').show()" onMouseOut="$myjquery(\'#pin-'.$commentID.'\').hide()" class="profile-stories phub2"><div class="showcasePhoto"><img src="http://www.kahub.com/l/'.$photoInfo['i-src'].'"></div><br clear="all" /><a href = "http://www.kahub.com/anyShare/xd_check?rURL=' . $storyData['s-url'] . '" target = "_blank" onclick = "StoryClick(' . $storyID . ')"><p style = "padding-top:8px;" class = "heavy phub">' . HelpingMethods::GetLimitedText($storyData['s-title'], 70) . '</a><div class="storyText phubpage"></div><div class="name"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$imgString.'</a><span class="realName"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$name.'</a></span> <span class="handle">(/'.$handle.')</span></div><div class="comment phub2">'.$commentData['c-text'].' '.$pin.'</div><div class="else hub pleft"><span class="comment">'.$replyText.'</span> &#183; '.$restate.' <span class="promote">'.$promote.'</span></div>'.$reply.'<div class="repliesinlinephub">'.$commentsBox.'</li>';
    		} else if($commentData['c-PostType']==3){
         		     $photoInfo = HelpingDBMethods::getPhotoInfo($commentID);
         		     $result .= '<li onMouseOver="$myjquery(\'#pin-'.$commentID.'\').show()" onMouseOut="$myjquery(\'#pin-'.$commentID.'\').hide()" class="profile-stories phub2"><div class="showcasePhoto"><img src="http://www.kahub.com/l/'.$photoInfo['i-src'].'"></div><br clear="all" /><a href = "http://www.kahub.com/anyShare/xd_check?rURL=' . $storyData['s-url'] . '" target = "_blank" onclick = "StoryClick(' . $storyID . ')"><p style = "padding-top:8px;" class = "heavy phub">' . HelpingMethods::GetLimitedText($storyData['s-title'], 70) . '</a><div class="storyText phubpage"></div><div class="name"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$imgString.'</a><span class="realName"><a href="http://www.kahub.com/l/phub.php?user='.$handle.'">'.$name.'</a></span> <span class="handle">(/'.$handle.')</span></div><div class="else hub pleft"><span class="comment">'.$replyText.'</span> &#183; '.$restate.' <span class="promote">'.$promote.'</span></div>'.$reply.'<div class="repliesinlinephub">'.$commentsBox.'</li>';
    		} else if($commentData['c-text']!="Bump"&&$commentData['c-reply']==0){
	...
	function getPhotoInfo($commentID)
	{
		$result = array();
		$Query = "SELECT * FROM tbl_buckets WHERE commentID = '$commentID'";	
		$QueryResult =  mysql_query($Query)or die(mysql_error());
		$row = mysql_fetch_array($QueryResult);
		if($row!=false)
		{
			$result['i-src'] = $row['ImageSrc'];
			$result['i-type'] = $row['ImageType'];
		}
		else
		{
			$result="pass";
	...
	function GetInterestsShared($MemberID)
	{
		...
		while($row!=false)
		{
		    ...
		    $urlhex = HelpingMethods::strToHex($storyData['s-url']);
		    $promoCount = HelpingDBMethods::GetPromoteCount($story);
		    $promote = "<span class='promoOnboard'><span class='promoteLoad' onClick='promoteShow(".$story.")'><a onClick=\"PromoteStory(".$story.", '".$urlhex."');\" class='promolink onboard' id='promote-story-link-".$story."'>Promote</a></span> (<span class='promo-num' id='promo-num-".$storyID."'>".$promoCount."</span>)</span>";
			$result .= '<li class="latestSharedGetStarted"><a href="http://www.kahub.com/anyShare/xd_check.php?ft=1&rURL='.$storyData['s-url'].'">'.$storyData['s-title'].'</a> '.$promote.'</li>';
			$row = mysql_fetch_array($QueryResult);
		}
		//echo $Query;
		return $result;
	...
	function GetCommentData($CommentID)
	{
		...
		if($row!=false)
		{
			...
			$result['c-PostType'] = $row['PostType'];
		}
		return $result;
	}
	function getTopicHubID($topic)
	{
			 
		$Query = "SELECT MemberID FROM tbl_hub WHERE (keyword  LIKE '%$topic%')";
		$QueryResult =  mysql_query($Query)or die(mysql_error());
		$row = mysql_fetch_array($QueryResult);
		if($row!=false)
		{
		    ...
		    $wikiData = HelpingDBMethods::wikidefinition($topic);
		    $created = HelpingDBMethods::isHubCreated($wikiData[3]);
		    if($created){
		        $create = false;
		        $result = HelpingDBMethods::getExactHubID($wikiData[3]);
		        return $result;
		    } 
		    if(strlen($topic)<3){
            	$create = false;
            ...
            if($create){
            	$result = MemberDBMethods::AddMemberHub($wikiData[3], "", "", "", "", "", "");
            	MemberDBMethods::addHub($wikiData[3], $result, $uid);
            }
            
		}
		return $result;
	}
	function isHubCreated($hubName)
    {
    	$result = false; 
    ...
    function getExactHubID($hubName)
    {
    	$result = false; 
    	$Query = "SELECT MemberID FROM tbl_hub WHERE (keyword  = '$hubName')";	
    	$QueryResult =  mysql_query($Query)or die(mysql_error());
    	$row = mysql_fetch_array($QueryResult);
    	if($row!=false)
    	{
