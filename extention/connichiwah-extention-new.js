	...
	function attachEvents()
	{
		...
		{
		    if($myJQuery(".div-float-main").size() == 0 || $myJQuery(".div-float-main:visible").size() == 0)
		    {
	...
	function showSelection(x, y) 
	{
	    ...
	    {				
		    setTimeout(function() 
	        { 
	            if(!isContainerFocused && txt.length <= 0 || txt2.length <= 0)
	            {			
	...
	function UIFace(text, x, y, timetofade, type)// for ajax
	{
		...
		$myJQuery.getJSON(WebServiceURL + "?method=" + type + "&callback=?", { url: url, text: text},  function(data){
			...
			$myJQuery("a.UI-Face-Image-Link").hover(function () 
	        {
		        ...
		        $myJQuery("#UI-Face-Image-Link-UserName" + un).show("slide", { direction: "left" }, 500);
	        }, function () 
	        {
	...
	function UIHoverboxCommentsMore(pno)
	{
		...
		$myJQuery.getJSON(WebServiceURL + "?method=" + type + "&callback=?", { url: url, text: text, pno:pno},  function(data){
			var comments = data.mycomments;	
			var heightOld = $myJQuery(".div-float-main").height();
			$myJQuery(".replies-container").html(comments);	
			var heightNew = $myJQuery(".div-float-main").height();
			var diff = 	heightNew - heightOld;
			var nh = $myJQuery(".div-float-main").offset().top - diff;
			$myJQuery(".div-float-main").css("top", nh);		
		});
	}
