...
$myJQuery(document).ready(function(){
	...
	urlquery=location.href.split("rURL=");
    var url = urlquery[1];
    url = encodeToHex(url);
...
function UIHoverboxShare(text, mx, my, timetofade, type)
{
	...
	$myJQuery(".connichiClass-Main587").fadeTo("fast", 0.3);
	$myJQuery.getJSON(WebServiceURL + "?method=" + type + "&callback=?", { url: url, text: text},  function(data){
		...
		$myJQuery(".fbshare").hide();
		$myJQuery(".share-btn").hide();
        $myJQuery(".lock-link").hide();
		$myJQuery(".share-fb").hide();
		$myJQuery(".share-fix").hide();
		$myJQuery("#commentstext").click(function(){
			$myJQuery(".fbshare").show();
			$myJQuery(".share-btn").show();
	        $myJQuery(".lock-link").show();
			$myJQuery(".share-fb").show();
			$myJQuery(".share-fix").show();
			var value = $(this).val();
			isContainerFocused = true;
			tempInactive = false;
			if (value=="" || value=="What do you think?"){
				value = $(this).val('');
			}
...
function Share()
{
    ...
    text = encodeToHex(text);
    var mycomments = $myJQuery("#commentstext").val();
	var fbshare = $myJQuery("#fbshare").val();
    $myJQuery("#commentstext").val("");
    var current_member_image = $myJQuery(".current-member-image").attr("src");
    var current_member_name = 'You';
    var heightOld = $myJQuery(".connichiClass-Main587").height();
...
else 
{
   alert('Post ID: ' + response.id);
    ...
    $myJQuery(".comment-wrapper").slideDown("fast", function(){
	    ...
	    $myJQuery.getJSON(WebServiceURL + "?method=" + type + "&callback=?", { url: url, title: mtitle, text: text, mycomments: mycomments, sharemode: my_preffered_share_method, sharewith: selected_source_id, fb: fbshare},  function(data){ 
		    //var comments = data.mycomments;
		    //ShowUIBox(comments, 100);
		    //$myJQuery(".connichiClass-Main587").remove();
...
function ShowUIBox(data, timetofade)// for ajax
{
	...
	$myJQuery(".connichiClass-Main587").fadeTo("fast", 1);
    ...
    }, function () 
    {
		...
		$myJQuery(".connichiClass-Main587").fadeTo("fast", 0.7);
		
		
        hideFaceOnUnfocus();  
    });

    $myJQuery(".reply-box").mouseover(function () 
    {
        isContainerFocused = true;
		tempInactive = false;
		console.log('mouse over!');
    ...
    $myJQuery(".white-box-172").mouseover(function () 
    {
        isContainerFocused = true;
		console.log('textbox!');
		$myJQuery(".connichiClass-Main587").fadeTo("fast", 1);
	...
	$myJQuery("#commentstext").focus(function () 
    {
        isContainerFocused = true;
		console.log('textbox!');
	...
	$myJQuery(".white-box-172").click(function () 
    {
        isContainerFocused = true;
		console.log('textbox!');
