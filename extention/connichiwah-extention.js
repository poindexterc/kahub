...
var WebServiceURL = "http://www.kahub.com/extention/webservice.php";
...
if(active_url.toLowerCase().search(rootURL + 'anyShare/story.php'.toLowerCase()) != -1)
{
    active_url = active_url.substring(active_url.indexOf('&rURL=') + 6);
    //alert('change active url : ' + active_url);
}
...
$myJQuery(document).ready(function(){
    ...
    var url = active_url;//window.location.href;
    url = encodeToHex(url);
    $myJQuery.getJSON(WebServiceURL + "?method=isInRestrictedDomain&callback=?", { url: url },  function(data){ 
        var result = data.result;
        //alert(data.activeUrl);        	        
        if(result == 'false' || result == false)
        {
            blocked = false;
            //active_url = data.activeUrl;
            //alert("Extention is Attached");            
            if(isAttacheAllowed)
            {  
...
function UILogin()
{
    ...
    window.open ("http://www.kahub.com/extention/login.php","mywindow","menubar=1,resizable=1,width=350,height=250"); 
    //jQuery.popupWindow({ 
...
function SourceSelected(ss)
{
    ...
    if(selected_source_id <= 0)
    {
		...
		$myJQuery("#person").addClass("active");
		$myJQuery("#friends").removeClass("active");
		$myJQuery("#world").removeClass("active");
    }
...
function PostReply(CommentID)
{
    ...
    url = encodeToHex(url);
    //text = encodeToHex(text);
    var mycomments = escape($myJQuery("#commentstext").val()); 
...
function Share()
{
    ...
    var current_member_name = 'You';
    var heightOld = $myJQuery(".connichiClass-Main587").height();
    $myJQuery(".replies-container").prepend('<div class = "comment-wrapper" style = "display:none;"><img alt="" src="' + current_member_image + '" class="UI-Face-Image fl"/><div class="comment-text fl"><span>' + current_member_name + '</span> said ' + unescape(mycomments) + '</div><div class="cl"></div></div>');
    $myJQuery(".comment-wrapper").slideDown("fast", function(){
        var heightNew = $myJQuery(".connichiClass-Main587").height();
	    var diff = 	heightNew - heightOld;
	    var nh = $myJQuery(".connichiClass-Main587").offset().top - diff;
...
function ShowUIBox(data, timetofade)// for ajax
{
    ...
    $myJQuery(".connichiClass-Main587").hover(function () 
    {
	...
	$myJQuery("#commentstext").mouseover(function () 
    {
        isContainerFocused = true;
		console.log('textbox!');
		$myJQuery('#commentstext').focus();
...
$myJQuery(document).click(function(event){
    console.log("click");
    if(!isContainerFocused){
        console.log("bad click");
         $myJQuery(".connichiClass-Main587").hide();
    }
...
$myJQuery(function() {
	$myJQuery( "#radio" ).buttonset();
});

function changeActive(changeTo){
	if(changeTo==1){
		UIPrivacySourceSelection();
		$myJQuery("#person").addClass("active");
		$myJQuery("#friends").removeClass("active");
		$myJQuery("#world").removeClass("active");
	} else if(changeTo==2){
		SourceSelected(2);
		$myJQuery("#person").removeClass("active");
		$myJQuery("#friends").addClass("active");
		$myJQuery("#world").removeClass("active");
	} else {
		my_preffered_share_method = 3;	    
		$myJQuery("#person").removeClass("active");
		$myJQuery("#friends").removeClass("active");
		$myJQuery("#world").addClass("active");
	}
