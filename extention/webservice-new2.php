...
{	
	...
	elseif($MethodName == "UI.Hoverbox.Share.Post")
	{
		...
		echo $_GET['callback'] . '(' . myWebservice::UIHoverboxSharePost($txt, $url, $title, $mycomments, $sharemode, $sharewith) . ');';
	}
...
class myWebservice
{
	...
	{		
		...
		if($DBConnection)
		{
			...
			{				
				...
				BadgeMethods::AssignMemberBadgesOnComment($MemberID, 'reply');
		...
		if($DBConnection)
		{
			...
			if($db_selected)
			{
				...
				if(HelpingDBMethods::isMemberTrusted($MemberID, $FriendID, $CategoryID) == false)
				{
					...
					BadgeMethods::AssignMemberBadgesOnTrust($MemberID, $storyID);
				}
				else
				{
	...
	function UIHoverboxSharePost($txt, $url, $title, $Comments, $sharemode, $sharewith)
	{
		$MemberID = $GLOBALS['user']->userID;
		$txt = HelpingMethods::hexToStr($txt);
		$txt = addslashes($txt);
		...
		if($DBConnection)
		{
			...
			{				
				...
				BadgeMethods::AssignMemberBadgesOnComment($MemberID, 'share');
				/////////////////////////////////////////////////////////////////////////////
				// function to share this comment with network or some source etc goes here//
				if($sharemode == 1)//1 for source 2 for network
				{
					//add privacy lock to comment for source selection
					HelpingDBMethods::InsertPrivacyLockOnComment($CommentID, $sharewith);
					// add notification fot this friend
					$data = array("AddressedTo" => $sharewith, "ActionBy" => $MemberID, "StoryID" => $storyID);
