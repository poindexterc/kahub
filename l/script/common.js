...
//var sWebServiceURL = "http://192.168.1.4/connichiwah/webService/gen.php";
//var rootURL = "http://192.168.1.4/connichiwah/";
    ...
    $myjquery.ajax(
        {
            ...
            success: function(data)
            {
                $myjquery(".dark-gray-heading").html($myjquery(data).find('title').text());
                var tags = $myjquery(data).find('tags').text().trim();
                $myjquery(".light-gray-box-img").attr("src", $myjquery(data).find('imgMember').text());
                $myjquery(".box-member-comment-text").html($myjquery(data).find('comment').text());
                if(tags.length >0) 
                {					
