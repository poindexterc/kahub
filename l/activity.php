...
if($user->is_loaded())
{
	...
	if($noOfFriends < 1)
	{
		...
		if($row != false)
		{
			$url = $row['Story_URL'];
			//echo '1--' . $url;
			header('Location:http://www.connichiwah.com/anyShare/index.php?ref=new_user_link&rand=0&rURL=' . $url);
		}
		else
		{
			$Query = "SELECT s.StoryID, s.Story_URL
	...
	{					
		...
		if($row != false)
		{
			$url = $row['Story_URL'];
			//echo '2--' . $url;
			header('Location:http://www.connichiwah.com/anyShare/index.php?ref=most_recent&rand=0&rURL=' . $url);
		}		
	}
}
else
