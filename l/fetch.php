...
$urldecode = urldecode($_REQUEST['url']);
$urlp = parse_url($urldecode);
$totalURL = $urlp['scheme'].$urlp['host'].$urlp['path'];
...
$inDB = HelpingDBMethods::storyInDB($urldecode);
if($inDB!=true){
	$story = file_get_contents('http://www.kahub.com/anyShare/read.php?rURL='.$url);
	HelpingDBMethods::insertStoryTextExtension($story, $url, $return_array['title']);
}
$return_array['storyID'] = HelpingDBMethods::GetStoryID($url);
// Parse Base
$base_override = false; 
